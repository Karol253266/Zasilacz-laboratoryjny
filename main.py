...
from PyQt5.QtCore import pyqtSlot, pyqtSignal, QRunnable, QThreadPool, QTimer, QThread, QCoreApplication, QObject, \
    QMutex, Qt
...
class dps_GUI(QMainWindow):
    ...
    def operating_mode(self):
        ...
        if value == 0:
            ...
            if self.pushButton_onoff.isChecked() and (
                    time.time() - self.pushButton_on_start_time > 5):  # adds 5sec delay, to prevent immediate switch OFF
    ...
    def pass_2_dps(self, function, cmd="r", value=0):
        ...
        if self.serialconnected != False:
            ...
            self.mutex.unlock()
            self.label_data_rate.setText(
                "Data Rate : %8.3fms" % ((time.time() - start) * 1000.0))  # display rate of serial comms
        return (a)
